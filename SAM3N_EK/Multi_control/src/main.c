...
#define PWM_FC_OUTPUT		PIO_PC21_IDX
...
#define GPIO_PORT_EVENT_1			(PIO_PC18_IDX)//PIN A
#define GPIO_PORT_EVENT_2			(PIO_PC19_IDX)//PIN B
#define GPIO_PORT_EVENT_3			(PIO_PC29_IDX)//PIN A
#define GPIO_PORT_EVENT_4			(PIO_PC30_IDX)//PIN B
#define GPIO_PORT_EVENT_5			(PIO_PC20_IDX)//PIN A
#define GPIO_PORT_EVENT_6			(PIO_PC31_IDX)//PIN B
...
{	
	...
	ant_states=actual_states; //Importantisimo actualizar los estados!!!
	
	
	
	duty_cicle=force2dutycicle(FC_ant);
	
	//set_count(&encoder_sample,ENC_FLOOR_DISTURBANCE,100*i);
	
	//set_count(&encoder_sample,ENC_POSPLATE,50*i);
...
int main (void)
{
	...
	//set_count(&encoder_count,ENC_MOTOR_DIFERENCIAL,77);
	//set_count(&encoder_count,ENC_FLOOR_DISTURBANCE,79);
	//set_count(&encoder_count,ENC_POSPLATE,71);
